    ...
    <article className={`product-card ${isCompact ? "compact-product-card" : ""} group flex h-full min-w-0 flex-col rounded-lg border border-black/7 bg-white p-3 shadow-card transition hover:-translate-y-1 hover:shadow-xl`}>
      ...
      <div className="flex min-w-0 flex-1 flex-col p-2">
        <div className="mt-4 flex flex-wrap gap-2">
          {badges.slice(0, isCompact ? 1 : 3).map((badge) => (
        ...
        <div className="mt-3 flex min-w-0 flex-wrap items-start justify-between gap-3">
          ...
          {!isCompact && <span className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-black ${isSoldOut ? "bg-red-100 text-red-700" : "bg-warm text-ink"}`}>{product.stockStatus}</span>}
        </div>
        ...
        {!isCompact && <p className="mt-1 text-sm font-bold text-ink/60">Confirm availability before payment</p>}
        <div className="mt-4 grid min-w-0 gap-2 text-base font-semibold text-ink/75">
          <span>{isCompact ? `${product.condition} | ${storage}` : `Storage: ${product.storage.join(", ")}`}</span>
          {!isCompact && <span>Colours: {product.colors.slice(0, 3).join(", ")}</span>}
        </div>
